...
public class Main : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (setting == myEnum.ForwardMotion)
        {
            ...
            Player.GetComponent<Movement_classic>().centerRotationLaterally = false;
            Player.GetComponent<Movement_classic>().restrict_horizontal = 0;
            Player.GetComponent<Movement_classic>().restrict_backwards = 0;
            Player.GetComponent<Movement_classic>().restrict_forwards = 1;
        ...
        if (setting == myEnum.ForwardAndBackward)
        {
            ...
            Player.GetComponent<Movement_classic>().centerRotationLaterally = false;
            Player.GetComponent<Movement_classic>().restrict_horizontal = 0;
            Player.GetComponent<Movement_classic>().restrict_backwards = 1;
            Player.GetComponent<Movement_classic>().restrict_forwards = 1;
            Reward_for_rotation = false;
        }
        ...
        if (setting == myEnum.AssistedLateral)
        {
            ...
            minimumDistance = 2;
            Player.GetComponent<Movement_classic>().centerRotationLaterally = false;
            Player.GetComponent<Movement_classic>().restrict_horizontal = 1;
            Player.GetComponent<Movement_classic>().restrict_backwards = 1;
        ...
        if (setting == myEnum.FixedLateral)
        {
            immediateReward = false;
            minimumDistance = 2;
            Player.GetComponent<Movement_classic>().centerRotationLaterally = true;
            Player.GetComponent<Movement_classic>().restrict_horizontal = 1;
            Player.GetComponent<Movement_classic>().restrict_backwards = 1;
            Player.GetComponent<Movement_classic>().restrict_forwards = 1;
            Reward_for_rotation = false;
        ...
        if (setting == myEnum.RotationOnly)
        {
            ...
            Player.GetComponent<Movement_classic>().restrict_horizontal = 1;
            Player.GetComponent<Movement_classic>().restrict_backwards = 0;
            Player.GetComponent<Movement_classic>().restrict_forwards = 0;
    ...
    void Update()
    {
        ...
        {
            if (target.GetComponent<Fruit_classic>().timeJuicy > maximumJuicyTime && state != 4)
            {
                state = 3;
            }

            if (target.GetComponent<Fruit_classic>().distance < minTargetDist)
            {
                minTargetDist = target.GetComponent<Fruit_classic>().distance;
                thisTargetNumber = iterator;
            }

            if (Reward_for_rotation)
            {
                target.GetComponent<Fruit_classic>().JuicyByAngle(angleForRotationReward);
            }
            iterator++;
        }
        ...
        if (abortIfFar && state != 4)
        {
            ...
            if (targets[thisTargetNumber].GetComponent<Fruit_classic>().distance > MaxDistAllowed)
            {
                state = 3;
            }
            ...
            {
                state = 3;
            } else if (minTargetDist > 10 && targets[thisTargetNumber].GetComponent<Fruit_classic>().playerAngle > 50)
            {
                state = 3;
            }
        ...
        {
            if (targets[thisTargetNumber].GetComponent<Fruit_classic>().playerAngle > MaxAngleAllowed)
            {
                state = 3;
            }
        ...
        if (abortIfBackwards && state != 4)
        {
            if ((GetComponent<Ardu>().ax2 < 0 || Input.GetAxisRaw("Vertical") < 0) && Player.GetComponent<Movement_classic>().restrict_backwards > 0)
            {
                state = 3;
            }
    ...
    public void reset()
    {
        ...
        for (int i = 0; i < targets.Length; i++)
        {
            ...
            targets[i].GetComponent<Fruit_classic>().angleJuice = false;
        }
